...
public class ImageController {
  ...
  /**
    * Retrieves a list of images similar to the one specified by ID.
    * 
    * This endpoint accepts an image ID, the number of similar images to return, and
    * a descriptor type. It returns a JSON array of similar images sorted by distance.
    * 
    * @param id The ID of the target image (path variable)
    * @param n The number of similar images to return (query parameter "number")
    * @param descriptor The descriptor type to use for comparison (query parameter "descriptor")
    * @return A ResponseEntity containing the JSON array of similar images, or an error message
    */
  ...
  public ResponseEntity<?> getSimilarImages(@PathVariable("id") long id,@RequestParam("number") int n,@RequestParam("descriptor") String descriptor) {
    try {
        ...
        List<Image> similarImages = imageService.findSimilarImages(id, n, descriptor);
        ArrayNode nodes = mapper.createArrayNode();
