...
{Sauvegarder une image filtrée dans la galerie personnelle}
{
...
{Structuration du code}
{
  ...
  \begin{itemize}
    \item \textbf{Frontend} : Les composants Vue.js est organisés dans le dossier \verb!frontend/src/components!, avec des fichiers comme \verb!Edit.vue! (édition d'images), \verb!Gallery.vue! (affichage des galeries), \verb!Login.vue! (authentification), etc. Les fichiers TypeScript pour la logique API est placés dans \verb!frontend/src!, comme \verb!http-api.ts! pour les appels au serveur. Les traductions sont stockées dans \verb!frontend/src/locale! (les fichiers \verb!fr.json!, \verb!en.json!, \verb!ar.json!).
