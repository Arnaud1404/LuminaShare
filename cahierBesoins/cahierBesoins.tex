...
{prévisualiser l'application d'un filtre sur une image}
{
  ...
  Dans une séction prédifinis, une image avec le filtre appliqué apparaîtra en temps réeel
}
{}
{}

\besoin{client:Metadonnées}
{Metadonnées d'une images}
{
  Affichier de manière visibles toutes les métadonnées (données EXIF) extractable de l'image
  \begin {itemize}
    \item size
    \item discription
    \item extension
