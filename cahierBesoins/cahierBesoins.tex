...
{
  L'utilisateur peut redimensionner une image sélectionnée à une taille standard (par exemple, 300x300 pixels) pour optimiser son stockage ou son affichage.
}
{
...
{
  Vérifier que l'image redimensionnée a bien les dimensions spécifiées (par exemple, 300x300 pixels) et que son contenu visuel est préservé.
}
...
{
  Tester qu'un pixel blanc (255, 255, 255) devient noir (0, 0, 0) et qu’un pixel de couleur (par exemple, R=200, G=150, B=50) devient sa complémentaire (R=55, G=105, B=205).
}
...
{
  Vérifier que pour un miroir horizontal, le pixel en position (0, 0) est déplacé à (largeur-1, 0), et pour un miroir vertical, le pixel en (0, 0) est déplacé à (0, hauteur-1).
}
...
{
  Vérifier que pour une rotation de 90° dans le sens horaire, les dimensions de l'image sont inversées (largeur devient hauteur, hauteur devient largeur) et que le contenu est correctement orienté.
}
